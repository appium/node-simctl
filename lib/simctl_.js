...
class Simctl {
  ...
  async exec (subcommand, opts = {}) {
    ...
    env = _.defaults(
      _.mapKeys(env, (value, key) => _.startsWith(key, SIMCTL_ENV_PREFIX) ? key : `${SIMCTL_ENV_PREFIX}${key}`),
      process.env);

    const execOpts = {
      timeout: asynchronous ? null : this.execTimeout,
      env,
    };
    try {
      return asynchronous
        ? new SubProcess(this.xcrun.path, args, execOpts)
        : await exec(this.xcrun.path, args, execOpts);
    } catch (e) {
      if (!this.logErrors) {
