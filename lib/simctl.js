...
async function getDevicesByParsing () {
  ...
  let iOSdeviceSectionRe = /-- iOS (.+) --(\n\s{4}.+)*/mg;
  let tvOSdeviceSectionRe = /-- tvOS (.+) --(\n\s{4}.+)*/mg;
  let deviceSectionRe = [iOSdeviceSectionRe, tvOSdeviceSectionRe];
  let matches = [];
  let match;
  deviceSectionRe.forEach(function (element) {
    match = element.exec(stdout);
...
async function getDevices (forSdk = null) {
  ...
  try {
    ...
    for (let [sdkName, entries] of _.toPairs(JSON.parse(stdout).devices)) {
      ...
      } else if (sdkName.indexOf('tvOS') === 0) {
        sdk = sdkName.replace('tvOS ', '');
      } else {
        continue;
      }
      let tempDevices = {};
      tempDevices[sdk] = entries.map((el) => {
        delete el.availability;
        return {...el, sdk};
      });
      if (devices[sdk]) {
        Object.assign(devices[sdk], tempDevices[sdk]);
      } else {
        Object.assign(devices, tempDevices);
      }
    }
  } catch (err) {
