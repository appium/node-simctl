...
async function getScreenshot (udid:string):string {
  let tempScreenshotDir = path.resolve(__dirname, 'tmp-screenshots');
  await fs.mkdir(tempScreenshotDir);
  let tempScreenshotPath = path.resolve(tempScreenshotDir, `temp-screenshot-${udid}.png`);
  await simExec('io', 0, [udid, 'screenshot', tempScreenshotPath]);
  let screenshotImg = await fs.readFile(tempScreenshotPath);
  await fs.rimraf(tempScreenshotDir);
