...
/**
 * Get the full list of runtimes, devicetypes, devices and pairs in JSON
 *
 * @return {Array<string>} List of the device types, runtimes devices and pairs JSO
 * @throws {Error} If the corresponding simctl command fails
 */
async function getSimctlList () {
  try {
     ...
     *   }
     */
    const fullList = JSON.parse(stdout.trim());
    return fullList;
  } catch (err) {
    let msg = `Unable to get simclt list: ${err.message}`;
    if (err.stderr) {
      msg = `${msg}. Stderr: ${err.stderr}`;
    }
    throw new Error(msg);
