...
async function startBootMonitor (udid, opts = {}) {
  ...
  let error = null;
  const bootMonitor = await simSubProcess('bootstatus', 0);
  bootMonitor.on('output', (stdout, stderr) => {
    ...
    if (stdout) {
      if (stdout.includes('Waiting on Data Migration') && onWaitingDataMigration) {
        ...
        onWaitingSystemApp();
      } else if (stdout.includes('Finished') || stdout.includes('Device already booted')) {
        if (onFinished) {
          onFinished();
        }
  ...
  if (!onFinished) {
    try {
      ...
      throw new Error(`The simulator ${udid} failed to finish booting after ${timeout}ms. Original command output: ${status}`);
    }
  }
  return bootMonitor;
}
