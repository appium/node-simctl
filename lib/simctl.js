...
import { logger, fs, tempDir, timing } from 'appium-support';
...
async function getAppearance (udid) {
  const {stdout} = await simExec('ui', 0, [udid, 'appearance'], {}, true);
  return (stdout || '').trim();
}
...
/**
 * Adds the given certificate to the Keyvhain Store on the simulator
 *
 * @since Xcode 11.4 SDK
 * @param {string} udid - Device UDID.
 * @param {string} certPath the full path to a valid .cert file containing
 ...
 * @since Xcode 11.4 SDK
 * @param {string} udid - Device UDID.
 * @param {Object} content - The object that describes Apple push notification content.
...
async function pushNotification (udid, content) {
  ...
  try {
    ...
    await fs.writeFile(dstPath, JSON.stringify(content), 'utf8');
...
export {
  ...
  grantPermission, revokePermission, resetPermission,
  addRootCertificate, addCertificate, resetKeychain,
  pushNotification,
};
