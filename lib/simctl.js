...
async function getPasteboard (udid:string, encoding = 'utf-8'):string {
  let {stdout} = await simExec('pbpaste', 0, [udid]);
  // The string received from exec has utf-8 encoding by default
  if (['utf-8', 'utf8'].indexOf(encoding.toLowerCase()) === -1) {
    stdout = Buffer.from(stdout, 'utf-8').toString(encoding);
  }
  return stdout || '';
}
...
export { installApp, removeApp, launch, spawn, spawnSubProcess, openUrl,
         ...
         bootDevice, setPasteboard, getPasteboard };
